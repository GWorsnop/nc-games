...
const { checkType } = require("../app/utility/check-type");
...
describe("check type", () => {
  ...
  test("returns an resolved promise, if passed an object containing key with string", () => {
    const object = { george: "hello" };
    expect(checkType(object)).resolves.toEqual(object);
  });
  test("returns an resolved promise, if passed an object containing multiple keys with only strings", () => {
    const object = { george: "hello", northcoder: "yeah" };
    expect(checkType(object)).resolves.toEqual(object);
  });
  test("returns an rejected promise, if passed an object containing multiple keys with only strings", () => {
    const object = { george: "hello", northcoder: true };
    expect(checkType(object)).rejects.toEqual({
      errorMessage:
